...
class AuthProvider extends ChangeNotifier {
    ...
    if (token != null && driverJson != null) {
      try {
        ...
        _driver = DriverModel.fromJson(
            jsonDecode(driverJson) as Map<String, dynamic>);
        notifyListeners();
      } catch (e) {
        await logout();
      }
  ...
  Future<bool> login(String username, String password) async {
    ...
    try {
      final driverData =
          await SupabaseDirectService.instance.login(username, password);

      if (driverData != null) {
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('auth_token',
            'token_${driverData['id_driver']}_${DateTime.now().millisecondsSinceEpoch}');
  ...
  Future<bool> updateProfile({
    ...
    try {
      final hasPhotoChanged =
          photoBase64 != null && photoBase64 != _driver!.photoUrl;
      debugPrint(
          'AuthProvider.updateProfile: hasPhotoChanged = $hasPhotoChanged');
      if (hasPhotoChanged) {
        debugPrint(
            'AuthProvider.updateProfile: photoBase64 length = ${photoBase64!.length}');
      }
      ...
      final success = await SupabaseDirectService.instance.updateDriverProfile(
        ...
        name,
        phone,
        hasPhotoChanged ? photoBase64 : null,
        vehicleType: vehicleType ?? _driver!.vehicleType,
        vehiclePlate: vehiclePlate ?? _driver!.vehiclePlate,
      );
