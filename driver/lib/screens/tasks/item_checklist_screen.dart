...
class _ItemChecklistScreenState extends State<ItemChecklistScreen>
  ...
  Future<void> _confirm(
    ...
    if (checklistType == 'pickup' || (task.type == 'delivery' && checklistType == 'delivery')) {
      Navigator.of(context).pushNamed(
        AppRoutes.documentation,
        arguments: {
          'taskId': task.id,
          'fromChecklist': true,
          'isDeparture': checklistType == 'delivery',
        },
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    final args = ModalRoute.of(context)!.settings.arguments as ChecklistArgs;
              ...
              itemBuilder: (context, index) {
                ...
                return _ChecklistItemTile(
                  ...
                  onToggle: () {
                    ...
                    .slideX(
                        begin: 0.15, duration: 300.ms, curve: Curves.easeOut);
              },
            ),
          ),
        ],
...
class _ChecklistHeader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isTaskDelivery = task.type == 'delivery';
    String headerTitle = '';
    String headerSubtitle = '';
    if (isTaskDelivery) {
      if (isDelivery) {
        headerTitle = '📦 Checklist Keluar Gudang';
        headerSubtitle = 'Pastikan barang lengkap sebelum meninggalkan gudang';
      } else {
        headerTitle = '🤝 Checklist Penyerahan Alat';
        headerSubtitle =
            'Pastikan barang diserahkan ke pelanggan dengan lengkap';
      }
    } else {
      if (isDelivery) {
        headerTitle = '🔄 Checklist Penjemputan';
        headerSubtitle = 'Pastikan semua barang dari lokasi event sudah dimuat';
      } else {
        headerTitle = '🏢 Checklist Masuk Gudang';
        headerSubtitle =
            'Pastikan semua barang kembali ke gudang dengan lengkap';
      }
    }
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        ...
                        style:
                            TextStyle(fontSize: 13, color: AppColors.grey500, fontWeight: FontWeight.w500),
                      ),
                    ],
                  ),
                ],
...
class _ConfirmDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isTaskDelivery = taskType == 'delivery';
    ...
    if (isTaskDelivery) {
      if (isDelivery) {
        ...
        content =
            'Semua barang sudah Anda cek dari gudang dan siap dikirim. Lanjutkan perjalanan?';
      } else {
        title = 'Selesaikan Pengiriman?';
        content =
            'Semua barang sudah Anda serahkan ke pelanggan dengan lengkap. Tandai tugas pengiriman ini selesai?';
      }
    } else {
      if (isDelivery) {
        title = 'Siap Berangkat?';
        content =
            'Semua barang sudah Anda cek dari lokasi event dan siap dibawa ke gudang. Lanjutkan perjalanan?';
      } else {
        title = 'Konfirmasi Selesai?';
        content =
            'Semua barang sudah Anda kembalikan ke gudang dengan lengkap. Tandai tugas penjemputan ini selesai?';
      }
    }
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                          isDelivery ? AppColors.primary : AppColors.warning,
                      foregroundColor: Colors.white,
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(12)),
                      padding: const EdgeInsets.symmetric(vertical: 12),
